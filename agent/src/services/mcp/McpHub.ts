...
export class McpHub {
	...
	private async connectToServer(
		...
		if (config.disabled) {
			Logger.log(`[CUSTOM LOG] MCP DEBUG: Creating disabled connection object for server "${name}"`)
		...
		try {
			...
			transport.send = async (message: any) => {
				Logger.log(`[CUSTOM LOG] MCP OUT: ${name}:`, JSON.stringify(message, null, 2))
			...
			Object.defineProperty(transport, "onmessage", {
				...
				set: (handler) => {
					originalOnMessage = (message: any) => {
						Logger.log(`[CUSTOM LOG] MCP IN: ${name}:`, JSON.stringify(message, null, 2))
			...
			Logger.log(`[CUSTOM LOG] MCP DEBUG: Setting up notification handlers for server: ${name}`)
			Logger.log(`[CUSTOM LOG] MCP DEBUG: Client instance:`, connection.client)
			Logger.log(`[CUSTOM LOG] MCP DEBUG: Transport type:`, config.type)
			...
			try {
				...
				connection.client.fallbackNotificationHandler = async (notification: any) => {
					Logger.log(`[CUSTOM LOG] MCP Fallback Notification: ${name}:`, JSON.stringify(notification, null, 2))
	...
	private async fetchToolsList(serverName: string): Promise<McpTool[]> {
		try {
			Logger.log(`[CUSTOM LOG] Fetching tools for ${serverName}...`)
			...
			if (connection.server.disabled || !connection.client) {
				Logger.log(`[CUSTOM LOG] Server ${serverName} is disabled or has no client.`)
				return []
			}
			...
			Logger.log(`[CUSTOM LOG] Tools response from ${serverName}:`, JSON.stringify(response, null, 2))
			...
			// Validate autoApprove entries against actual tool names (invariant X-3)
			// Mismatches cause silent 600s hangs in headless mode
			if (autoApproveConfig.length > 0) {
				const actualToolNames = new Set(tools.map((t) => t.name))
				const orphanedApprovals = autoApproveConfig.filter((name: string) => !actualToolNames.has(name))
				if (orphanedApprovals.length > 0) {
					Logger.warn(
						`[MCP] Server "${serverName}": ${orphanedApprovals.length} autoApprove entries ` +
							`do not match any actual tool: [${orphanedApprovals.join(", ")}]. ` +
							`These tools will NOT be auto-approved. Check for typos or renamed tools.`,
					)
				}
			}
			...
			Logger.log(`[CUSTOM LOG] Error fetching tools for ${serverName}:`, error)
			return []
		}
	}
