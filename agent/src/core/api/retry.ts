			...
			for (let attempt = 0; attempt < maxRetries; attempt++) {
				try {
					...
					const isSocketError =
						...
						(error?.message && typeof error.message === "string" && error.message.includes("UND_ERR_SOCKET"))
					const isLastAttempt = attempt === maxRetries - 1

					if ((!isRateLimit && !isSocketError && !retryAllErrors) || isLastAttempt) {
						throw error
					}

					if (isSocketError) {
						Logger.log(`[Retry] UND_ERR_SOCKET detected on attempt ${attempt + 1}/${maxRetries}, retrying...`)
					}
