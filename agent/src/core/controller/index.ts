...
export class Controller {
	...
	*/
	...
	async initTask(
		...
		// Force backgroundExec in non-VS-Code hosts (Theia, GeologyIDE, etc.)
		// Shell integration is never available there. Env vars may not propagate
		// to extension hosts, so also check vscode.env.appName.
		const isNonVscode1 = !!(
		...
		const vscodeTerminalExecutionMode = isNonVscode1
			? ("backgroundExec" as const)
			: this.stateManager.getGlobalStateKey("vscodeTerminalExecutionMode")
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		const agentMode = this.stateManager.getGlobalSettingsKey("agentMode")
		...
		const vscodeTerminalExecutionMode = isNonVscode2
			? ("backgroundExec" as const)
			: this.stateManager.getGlobalStateKey("vscodeTerminalExecutionMode")
		const defaultTerminalProfile = this.stateManager.getGlobalSettingsKey("defaultTerminalProfile")
		const isNewUser = this.stateManager.getGlobalStateKey("isNewUser")
		...
		// Disabled for custom Geocluster IDE — upstream Cline announcements are not relevant
		const shouldShowAnnouncement = false
