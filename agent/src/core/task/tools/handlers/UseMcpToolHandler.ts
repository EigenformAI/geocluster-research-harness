...
import { globMatch } from "@core/prompts/system-prompt/components/mcp"
...
import { getSpecialistMcpFilter } from "../utils/specialist-filter"
...
export class UseMcpToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		// Execution-time MCP filter enforcement (invariant L3-2)
		// Prevents a hallucinating model from calling tools filtered from its prompt
		const mcpFilter = getSpecialistMcpFilter(config.taskState.activeSpecialist)
		if (mcpFilter) {
			if (mcpFilter.allowedToolPatterns?.length) {
				const isAllowed = mcpFilter.allowedToolPatterns.some((p) => globMatch(p, tool_name))
				if (!isAllowed) {
					config.taskState.consecutiveMistakeCount++
					return formatResponse.toolError(
						`Tool "${tool_name}" is not available to the ${config.taskState.activeSpecialist} specialist. ` +
							`Allowed patterns: ${mcpFilter.allowedToolPatterns.join(", ")}`,
					)
				}
			}
			if (mcpFilter.blockedToolPatterns?.length) {
				const isBlocked = mcpFilter.blockedToolPatterns.some((p) => globMatch(p, tool_name))
				if (isBlocked) {
					config.taskState.consecutiveMistakeCount++
					return formatResponse.toolError(
						`Tool "${tool_name}" is blocked for the ${config.taskState.activeSpecialist} specialist.`,
					)
				}
			}
		}
		...
		if (config.callbacks.shouldAutoApproveTool(block.name) && isToolAutoApproved !== false) {
			// Auto-approval flow
		...
		try {
			...
			return formatResponse.toolError(`Error executing MCP tool "${tool_name}": ${message}`)
		}
	}
}
