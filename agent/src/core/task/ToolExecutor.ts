...
import { ActivateSpecialistHandler } from "./tools/handlers/ActivateSpecialistHandler"
...
import { DispatchSpecialistHandler } from "./tools/handlers/DispatchSpecialistHandler"
...
import { RequestCapabilityHandler } from "./tools/handlers/RequestCapabilityHandler"
...
import { getSpecialistAllowedTools } from "./tools/utils/specialist-tool-guard"
...
export class ToolExecutor {
	...
	constructor(
		...
		) => Promise<[boolean, any, string[]?, boolean?]>,
	...
	private registerToolHandlers(): void {
		...
		this.coordinator.register(new ActivateSpecialistHandler())
		this.coordinator.register(new DispatchSpecialistHandler())
		this.coordinator.register(new RequestCapabilityHandler())
	}
	...
	private pushToolResult = (content: ToolResponse, block: ToolUse) => {
		...
		Logger.log(`[CUSTOM LOG] Tool Results: ${block.name}:`, content)
	...
	private async execute(block: ToolUse): Promise<boolean> {
		...
		if (!block.partial) {
			Logger.log("[CUSTOM LOG] Tool Execute: ${block.name} arguments:", JSON.stringify(block.params, null, 2))
		}
		...
		try {
			...
			// Specialist tool guard: block tools not in the specialist's allowed set (invariant L3-3)
			if (this.taskState.activeSpecialist) {
				const allowedTools = getSpecialistAllowedTools(this.taskState.activeSpecialist)
				if (allowedTools && !allowedTools.has(block.name)) {
					const errorMsg = `Tool "${block.name}" is not available to the ${this.taskState.activeSpecialist} specialist. Available tools: ${[...allowedTools].join(", ")}`
					if (!block.partial) {
						this.pushToolResult(formatResponse.toolError(errorMsg), block)
					}
					return true
				}
			}
