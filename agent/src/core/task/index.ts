...
import { isSubagentCommand } from "@/integrations/cli-subagents/subagent_command"
...
import { getSpecialistType, isClineCliInstalled, isCliSubagentContext } from "@/utils/cli-detector"
...
export class Task {
	...
	): Promise<[boolean, ClineToolResponseContent, string[]?, boolean?]> {
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		// Detect specialist context from env var (set when spawned via dispatch_specialist)
		const specialistType = getSpecialistType()
		if (specialistType && !this.taskState.activeSpecialist) {
			this.taskState.activeSpecialist = specialistType
			Logger.log(`[Specialist] Detected from env CLINE_SPECIALIST=${specialistType}`)
		}
		if (this.taskState.activeSpecialist) {
			Logger.log(`[Specialist] Active specialist for this turn: ${this.taskState.activeSpecialist}`)
		}
		...
		const promptContext: SystemPromptContext = {
			...
			isCliSubagent,
			isCliEnvironment,
			activeSpecialist: this.taskState.activeSpecialist || undefined,
			agentMode: this.stateManager.getGlobalSettingsKey("agentMode") || "geology",
			specialistHistory: this.taskState.specialistHistory.length > 0 ? this.taskState.specialistHistory : undefined,
		...
		Logger.log("[CUSTOM LOG] Api Request: System Prompt:", systemPrompt)
		Logger.log(
			"[CUSTOM LOG] Api Request: Context History (including files):",
			JSON.stringify(contextManagementMetadata.truncatedConversationHistory, null, 2),
		)
		Logger.log("[CUSTOM LOG] Api Request: Tools:", JSON.stringify(tools, null, 2))
	...
	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		...
		const busyTerminals = this.terminalManager.getTerminals(true).filter((t) => !isSubagentCommand(t.lastCommand))
		const inactiveTerminals = this.terminalManager.getTerminals(false).filter((t) => !isSubagentCommand(t.lastCommand))
