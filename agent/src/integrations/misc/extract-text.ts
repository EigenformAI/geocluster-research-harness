import { applyFallbackTokenSafetyNet, isDataFile, truncateDataFilePreview } from "@core/task/tools/utils/DataFileGuard"
...
export async function callTextExtractionFunctions(filePath: string): Promise<string> {
	...
	switch (fileExtension) {
		...
		default:
			...
			if (fileBuffer.byteLength > 5 * 1000 * 1024) {
				// 5MB limit (5 * 1000 * 1024 bytes, decimal MB)
...
export async function processFilesIntoText(files: string[]): Promise<string> {
	const fileContentsPromises = files.map(async (filePath) => {
		try {
			...
			//}
			let content = await extractTextFromFile(filePath)

			// Apply truncation guards (same logic as read_file tool, but with hardcoded budget
			// since user-attached files don't have API context for context window info)
			if (isDataFile(filePath)) {
				const result = truncateDataFilePreview(content, filePath)
				if (result.wasTruncated) {
					content = result.content
				}
			}
			const safetyResult = applyFallbackTokenSafetyNet(content)
