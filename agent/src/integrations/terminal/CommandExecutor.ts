...
export class CommandExecutor {
	 ...
	 * @returns [userRejected, result, rawOutputLines?] tuple. Third element is raw
	 *          (pre-filter) output lines for subagent commands, used for token extraction.
	 */
	async execute(
		command: string,
		...
		if (this.wasCancelledExternally) {
			const outputSoFar =
				result.outputLines.length > 0
					? `\nOutput captured before cancellation:\n${manager.processOutput(result.outputLines, undefined, isSubagent)}`
					: ""
			return [true, `Command was cancelled by the user.${outputSoFar}`]
		}

		// For subagent commands, include raw output lines as a third tuple element
		// so callers (e.g., DispatchSpecialistHandler) can extract token usage
		// from unfiltered output. The processed result (second element) has been
		// through filterSubagentOutput() which strips api_req_started JSON lines.
		return isSubagent
			? [result.userRejected, result.result, result.outputLines, result.completed]
			: [result.userRejected, result.result]
	}
