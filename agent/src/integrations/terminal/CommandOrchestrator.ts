...
export async function orchestrateCommandExecution(
	...
	process.once("error", (err: Error) => {
		Logger.error(`[CommandOrchestrator] Process error: ${err.message}`)
		clearCommandState()
	})
	process.catch(() => {
		clearCommandState()
	 ...
	 *
	 * If the process has already completed, uses say() instead to avoid blocking.
	 */
	const flushBuffer = async (force = false) => {
		...
		if (!didContinue) {
			// If the process already completed, use non-blocking say() instead of ask().
			// This prevents the orchestrator from getting stuck waiting for user interaction
			// when the command has already finished (common in backgroundExec/Theia mode
			// where child_process fires "close" before all buffered output is flushed).
			if (completed) {
				await callbacks.say("command_output", chunk)
				return
			}

			// In standalone/backgroundExec mode (Theia, CLI), always use non-blocking say().
			// The terminal is hidden — there's no visible terminal for "Proceed While Running"
			// to make sense. The 30-second timeout provides automatic background detachment.
			// Using ask() here would create a blocking dialog that prevents the conversation
			// from proceeding even after the child process completes.
			if (terminalType === "standalone") {
				await callbacks.say("command_output", chunk)
				return
			}
			...
			try {
				...
				if (response === "yesButtonClicked") {
					...
					if (onProceedWhileRunning) {
						...
						const result = terminalManager.processOutput(outputLines, undefined, isSubagentCommand)
	...
	process.once("no_shell_integration", async () => {
		// Suppress shell integration warnings in standalone/backgroundExec mode and Theia.
		// StandaloneTerminalProcess never emits this event, but guard defensively.
		// In Theia, shell integration is never available and the warnings are just noise.
		if (
			terminalType === "standalone" ||
			globalThis.process.env.THEIA_WEBVIEW_EXTERNAL_ENDPOINT ||
			globalThis.process.env.THEIA_DEFAULT_PLUGINS
		) {
			Logger.log(
				`[Terminal] Suppressed shell_integration_warning (terminalType=${terminalType}, ` +
					`THEIA_WEBVIEW=${!!globalThis.process.env.THEIA_WEBVIEW_EXTERNAL_ENDPOINT}, ` +
					`THEIA_PLUGINS=${!!globalThis.process.env.THEIA_DEFAULT_PLUGINS})`,
			)
			return
		}
		Logger.log(
			`[Terminal] Showing shell_integration_warning (terminalType=${terminalType}, ` +
				`THEIA_WEBVIEW=${!!globalThis.process.env.THEIA_WEBVIEW_EXTERNAL_ENDPOINT}, ` +
				`THEIA_PLUGINS=${!!globalThis.process.env.THEIA_DEFAULT_PLUGINS})`,
		)
	...
	if (isWritingToFile) {
		...
		result = terminalManager.processOutput(summaryLines, undefined, isSubagentCommand)
		resultOutputLines = summaryLines
	} else {
		result = terminalManager.processOutput(outputLines, undefined, isSubagentCommand)
