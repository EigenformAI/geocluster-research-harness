...
import { AgentMode, Mode, OpenaiReasoningEffort } from "./storage/types"
...
export interface ClineSaySpecialistDispatch {
	specialist: string // "dataops" | "transform" | "analytics" | "geoviz" | "extended"
	taskId: string // e.g. "dataops_1738000000000"
	objective: string // truncated to ~200 chars for display
	status: "running" | "completed" | "error" | "timeout"
	durationMs?: number // populated on completion
	errorSummary?: string // populated on error/timeout
	// Per-dispatch token usage totals (summed from child's api_req_started messages)
	tokensIn?: number
	tokensOut?: number
	cacheWrites?: number
	cacheReads?: number
	cost?: number
	toolCalls?: string[] // tools used by the specialist (e.g. "geocluster-mcp:inspect_dataset", "read_file")
}
