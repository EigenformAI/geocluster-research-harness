	...
	private readFromDisk(): Record<string, T> {
		try {
			if (fs.existsSync(this.fsPath)) {
				...
				if (!content.trim()) {
					Logger.error(`[${this.name}] file is empty at ${this.fsPath}, starting fresh`)
					return {}
				}
				return JSON.parse(content)
			}
		} catch (error) {
			Logger.error(`[${this.name}] failed to read/parse ${this.fsPath} (possible corruption), starting fresh:`, error)
		}
		return {}
	}

	private writeToDisk(): void {
		const tmpPath = `${this.fsPath}.tmp.${process.pid}.${Date.now()}`
		try {
