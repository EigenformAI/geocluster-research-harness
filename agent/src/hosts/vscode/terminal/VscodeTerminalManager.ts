...
import { filterSubagentOutput } from "@/integrations/terminal/output-filters"
import {
...
export class VscodeTerminalManager implements ITerminalManager {
	...
	public processOutput(outputLines: string[], overrideLimit?: number, isSubagentCommand?: boolean): string {
		...
		const filteredLines = isSubagentCommand ? filterSubagentOutput(outputLines) : outputLines
		...
		if (filteredLines.length > limit) {
			const halfLimit = Math.floor(limit / 2)
			const start = filteredLines.slice(0, halfLimit)
			const end = filteredLines.slice(filteredLines.length - halfLimit)
			return `${start.join("\n")}\n... (output truncated) ...\n${end.join("\n")}`.trim()
		}
