...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Empty tree provider for the "Upload Files" explorer section. The view has no
	// items, so VS Code renders the `viewsWelcome` content from package.json (the
	// drag-to-upload hint) in its place. Drops are handled by code-server's native
	// explorer file-upload — this view is purely instructional.
	context.subscriptions.push(
		vscode.window.registerTreeDataProvider("geocluster.uploadHelp", {
			getChildren: () => [],
			getTreeItem: (element: vscode.TreeItem) => element,
		}),
	)

	// Secondary Explorer views (like Outline/Timeline) start collapsed by default and
	// the manifest `visibility` hint is not reliably honored, so reveal the upload
	// section once on startup. The short delay lets the workbench restore its saved
	// view layout first; otherwise the restore overrides the expand. Focus only moves
	// within the left Explorer — the Geology Agent chat panel (separate container) is
	// untouched.
	const revealUploadHelp = setTimeout(() => {
		vscode.commands.executeCommand("geocluster.uploadHelp.focus").then(undefined, () => {})
	}, 1500)
	context.subscriptions.push({ dispose: () => clearTimeout(revealUploadHelp) })
	...
	*/
	...
	if (IS_DEV) {
		vscode.commands.executeCommand("setContext", "geology-agent.isDevMode", IS_DEV)
		...
		vscode.commands.registerCommand(commands.TerminalOutput, async () => {
			...
			try {
				...
				try {
					...
					// If command fails, just restore clipboard and return
