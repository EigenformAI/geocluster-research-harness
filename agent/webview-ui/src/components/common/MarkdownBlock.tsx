...
/**
 * Custom remark plugin that tags blockquotes with data-content-type attributes
 * for visual differentiation of data-sourced facts, interpretations, and assumptions.
 * Follows the same node.data.hProperties pattern as remarkMarkPotentialFilePaths.
 */
const remarkContentTypeMarker = () => {
	return (tree: Node) => {
		visit(tree, "blockquote", (node: any) => {
			...
			if (firstInline.type === "strong" && firstInline.children?.[0]?.value?.startsWith("Data:")) {
				contentType = "data"
			} else if (firstInline.type === "emphasis") {
				const val = firstInline.children?.[0]?.value || ""
				if (val.startsWith("Interpretation:")) contentType = "interpretation"
				else if (val.startsWith("Assumption:")) contentType = "assumption"
			}

			if (contentType) {
				node.data = node.data || {}
				node.data.hProperties = node.data.hProperties || {}
				node.data.hProperties["data-content-type"] = contentType
			}
		})
	}
}
