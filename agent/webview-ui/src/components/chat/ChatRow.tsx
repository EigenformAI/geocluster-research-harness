...
export const ChatRowContent = memo(
	({
		...
		const { backgroundEditEnabled, mcpServers, mcpMarketplaceCatalog, vscodeTerminalExecutionMode, clineMessages } =
			useExtensionState()
				...
				switch (message.say) {
					...
					case "error_retry":
						try {
							...
							const { attempt, maxAttempts, delaySeconds, failed, errorMessage: _errorMessage } = retryInfo
							const isFailed = failed === true

							return (
								<div className="flex flex-col gap-2">
									{/* errorMessage commented out — raw JSON not user-friendly, ErrorRow above already shows clean message */}
									{/* {_errorMessage && (
										<p className="m-0 whitespace-pre-wrap text-error wrap-anywhere text-xs">{_errorMessage}</p>
									)} */}
					...
					case "specialist_dispatch": {
						let dispatchInfo: ClineSaySpecialistDispatch = {
							specialist: "unknown",
							taskId: "",
							objective: "",
							status: "running",
						...
						try {
							if (message.text) {
								dispatchInfo = JSON.parse(message.text)
							}
						} catch {
							// Use defaults if parsing fails
						}
						const dispatchCancelled =
							dispatchInfo.status === "running" &&
							(!isLast ||
								lastModifiedMessage?.ask === "resume_task" ||
								lastModifiedMessage?.ask === "resume_completed_task")
						const isRunning = dispatchInfo.status === "running" && !dispatchCancelled
						const isDispatchError = dispatchInfo.status === "error" || dispatchInfo.status === "timeout"
						const durationSec = dispatchInfo.durationMs ? Math.round(dispatchInfo.durationMs / 1000) : undefined
						const specialistLabel = dispatchInfo.specialist.charAt(0).toUpperCase() + dispatchInfo.specialist.slice(1)
						return (
							<div className="bg-code flex flex-col border border-editor-group-border rounded-sm py-2.5 px-3">
								<div className="flex items-center">
									{isRunning ? (
									...
									<code className="bg-quote rounded-sm py-0.5 px-1.5 mr-2 text-xs">{specialistLabel}</code>
									<span className="font-semibold">
										{isRunning
											? "Specialist running"
											: dispatchInfo.status === "timeout"
												? "Specialist timed out"
												: isDispatchError
													? "Specialist failed"
													: dispatchCancelled
														? "Specialist interrupted"
														: "Specialist completed"}
									</span>
									{durationSec !== undefined && <span className="opacity-60 ml-2 text-xs">{durationSec}s</span>}
								</div>
								{dispatchInfo.objective && (
									<div className="opacity-70 ml-6 mt-1.5 text-xs break-words">{dispatchInfo.objective}</div>
								)}
								{!isRunning && (dispatchInfo.tokensIn || dispatchInfo.tokensOut || dispatchInfo.cost) && (
									<div className="opacity-60 ml-6 mt-1 text-xs flex gap-3">
										{(dispatchInfo.tokensIn !== undefined || dispatchInfo.tokensOut !== undefined) && (
											<span>
												Tokens: {(dispatchInfo.tokensIn ?? 0).toLocaleString()} in /{" "}
												{(dispatchInfo.tokensOut ?? 0).toLocaleString()} out
											</span>
										)}
										{dispatchInfo.cost !== undefined && dispatchInfo.cost > 0 && (
											<span>Cost: ${dispatchInfo.cost.toFixed(4)}</span>
										)}
									</div>
								)}
								{isDispatchError && dispatchInfo.errorSummary && (
									<div className="opacity-80 ml-6 mt-1.5 text-error break-words text-xs">
										{dispatchInfo.errorSummary}
									</div>
								)}
							</div>
						)
