 ...
 * Sums all api_req_started and specialist_dispatch costs in between.
...
export function findNextSegmentCost(checkpointTs: number, allMessages: ClineMessage[]): number | undefined {
	...
	// Sum all api_req_started and specialist_dispatch costs between this checkpoint and the end
	let totalCost = 0
	for (let i = checkpointIndex + 1; i < endIndex; i++) {
		const msg = allMessages[i]
		if ((msg.say === "api_req_started" || msg.say === "specialist_dispatch") && msg.text) {
			try {
				const info = JSON.parse(msg.text)
