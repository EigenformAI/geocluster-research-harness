...
import { AgentMode, Mode } from "@shared/storage/types"
...
const GEOLOGY_MODE_COLOR = "var(--vscode-charts-green, #4caf50)"
...
})<{ modeIndex: number; sliderColor: string }>`
	position: absolute;
	height: 100%;
	...
	(
		...
		const setAgentMode = useCallback(
			(newMode: AgentMode) => {
				...
				StateServiceClient.updateSettings(UpdateSettingsRequest.create({ agentMode: newMode })).catch((error) => {
				...
				<div className="flex justify-between items-center -mt-[2px] px-3 pb-2">
					...
					<Tooltip>
						<TooltipContent className="text-xs px-2 flex flex-col gap-1" side="top">
							{agentMode === "geology"
								? "Geology mode: geological analysis with specialist delegation"
								: agentMode === "report-analysis"
									? "Report Analysis mode: answer the stage-specific Mining-evals questions about an extracted report"
									: "Standard mode: general-purpose coding assistant"}
						</TooltipContent>
						<TooltipTrigger>
							<SwitchContainer data-testid="agent-mode-switch" disabled={false}>
								<AgentModeSlider
									modeIndex={Math.max(
										0,
										AGENT_MODES.findIndex((m) => m.mode === agentMode),
									)}
									sliderColor={
										(AGENT_MODES.find((m) => m.mode === agentMode) ?? AGENT_MODES[0]).color
									}
								/>
								{AGENT_MODES.map(({ mode, label }) => (
									<div
										...
										className={cn(
											"pt-0.5 pb-px px-2 z-10 text-xs text-center bg-transparent cursor-pointer",
											agentMode === mode ? "text-white" : "text-input-foreground",
										)}
										key={mode}
										onClick={() => setAgentMode(mode)}
										role="switch"
										style={{ width: `${100 / AGENT_MODES.length}%` }}>
										{label}
									</div>
								))}
							</SwitchContainer>
