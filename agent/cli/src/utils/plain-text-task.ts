...
export async function runPlainTextTask(options: PlainTextTaskOptions): Promise<boolean> {
	...
	const processMessage = (message: ClineMessage) => {
		...
		// Allow re-emission of api_req_started when text changes (e.g., token usage added after streaming)
		if (processedMessages.has(ts)) {
			if (!(message.say === "api_req_started" && message.text !== processedMessages.get(ts))) {
				return
			}
		}
