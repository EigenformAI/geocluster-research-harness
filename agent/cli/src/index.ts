...
/**
 * Configure the CLI from environment variables.
 *
 * OPENROUTER_API_KEY pre-configures the OpenRouter provider without user
 * interaction (mirrors configureOpenRouterFromEnv() in src/common.ts).
 *
 * When the CLI is spawned as a specialist child, the parent extension passes
 * CLINE_PARENT_MODEL_ID / CLINE_PARENT_PROVIDER so the child uses the same
 * model the user selected, and mirrors the active provider's API key into
 * <CLINE_DATA_DIR>/secrets.json before spawning (see DispatchSpecialistHandler).
 * Parent inheritance therefore must apply even when no env API key is set.
 */
async function configureFromEnvCli(): Promise<void> {
	const stateManager = StateManager.get()

	const envApiKey = process.env.OPENROUTER_API_KEY
	const parentModelId = process.env.CLINE_PARENT_MODEL_ID
	const parentProvider = process.env.CLINE_PARENT_PROVIDER as ApiProvider | undefined

	let configured = false

	if (envApiKey) {
		// Persist the env key (skip the write if unchanged)
	...
	// Inherit the parent's provider + model selection when spawned as a
	// specialist child. Applies regardless of env API key — the credential
	// arrives via the mirrored secrets.json in CLINE_DATA_DIR.
	if (parentModelId && parentProvider) {
		const actModelKey = getProviderModelIdKey(parentProvider, "act")
		const planModelKey = getProviderModelIdKey(parentProvider, "plan")
		if (actModelKey) stateManager.setGlobalState(actModelKey, parentModelId)
		if (planModelKey) stateManager.setGlobalState(planModelKey, parentModelId)
		stateManager.setGlobalState("actModeApiProvider", parentProvider)
		stateManager.setGlobalState("planModeApiProvider", parentProvider)
		configured = true
	}
...
program
	...
	.action(async (prompt, options) => {
		...
		// Error if stdin was piped but empty AND no prompt argument was provided.
		// (e.g., `echo "" | cline` — user intended to pipe content but sent nothing)
		// When a prompt argument exists, empty stdin is expected — the subprocess
		// was spawned with stdio: ["ignore", ...] so stdin closes immediately.
